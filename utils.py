...
class GraphProcessor:
    ...
    def sample_sub(self, data, mean_error_list, threshold):
        ...
        for i in tqdm(range(len(center_nodes))):
    ...
    def anomaly_group_injector(self, G: Data, ano_sub: dict):
        ...
        for idx in range(tol_count):
            ...
            random_node = x[np.random.choice(G.num_nodes, 1, replace=False)[0]]
            x[cand_node] = random_node
            ...
            while True:
                ...
                for cn in cand_neis:
                    if cn not in neis:
                        size -= 1
                        x[cn] = np.random.normal(x.mean(), x.std(), x.shape[1])
...
def preprocess_data(data, type='s'):
    ...
    A = to_dense_adj(edge_index, max_num_nodes=data.num_nodes)[0]
    A_array = A.cpu().numpy()
    ...
    for i in np.arange(len(A_array)):
        ...
        sub_graphs.append(G.subgraph(s_indexes))#加入节点自身和邻居
#遍历所有子图，收集每个子图的节点列表、邻接矩阵和边的数量
